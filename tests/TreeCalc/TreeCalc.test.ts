//@ts-nocheck
import {ParseError, parseExpr} from "./gen/TreeCalcParser";
import {generateListString, generateListStringWithBrackets} from "../testGenerator";
import {convertTokens, evaluateExpression, TreeCalc} from "../../utils/parseTree.ts";

function t(s: string): number {
  let {val} = parseExpr(s)
  return evaluateExpression(convertTokens(val))
}
